...
void pcapPrepareARPHandler(const char *deviceName) {
  ...
  if ((pcap_set_snaplen(pcapARPHandler, SNAPLEN)) < 0) {
    throwErrorScreen(pcap_geterr(pcapARPHandler), PCAP_HANDLER_SET_SNAPLEN);
  }
  if ((pcap_set_promisc(pcapARPHandler, PROMISC)) < 0) {
    throwErrorScreen(pcap_geterr(pcapARPHandler), PCAP_HANDLER_SET_PROMISC);
  }
  if ((pcap_set_timeout(pcapARPHandler, TO_MS)) < 0) {
    throwErrorScreen(pcap_geterr(pcapARPHandler), PCAP_HANDLER_SET_TIMEOUT);
  }
  if ((pcap_activate(pcapARPHandler)) < 0) {
    throwErrorScreen(pcap_geterr(pcapARPHandler), PCAP_HANDLER_ACTIVATE);
  }
}